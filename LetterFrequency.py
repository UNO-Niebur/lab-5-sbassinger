...
#Date:02/22/2026
#Assignment:Lab 5 - Letter Frequency
...
def countLetters(message):
    ...
    for letter in message:
        if (alpha.find(letter) >= 0):
            spot = alpha.find(letter) #Find the position in the alphabet
            freq[spot] = freq[spot] + 1     #Increase the frequency at that position. If position was 5, then frequencies[5] = frequencies[5] + 1
